...
DETECTION_FILE = "target_detection.json"  # Path to your detection file
...
class DroneController:
    ...
    async def move_direction(self, direction, value):
        ...
        velocity_map = {
            ...
            "turn_cw":   (0.0, 0.0, 0.0, YAW_SPEED),    # New clockwise turn
            "turn_ccw":  (0.0, 0.0, 0.0, -YAW_SPEED),   # New counterclockwise turn
        }
        ...
        # For yaw and turn commands, duration is angle / yaw_rate
        if direction in ["yaw_right", "yaw_left", "turn_cw", "turn_ccw"]:
    ...
    TARGET_GPS_FILE = "target_gps.json"  # put near DETECTION_FILE

    # ----------------------- Go to object-------------------------------------
    async def goto_object(self, obj_class: str):
        """
        Fly to the highest‑confidence object whose class matches <obj_class>
        stored in target_gps.json
        """
        obj_class = obj_class.lower()
        TARGET_GPS_FILE = "target_gps.json"

        if not os.path.exists(TARGET_GPS_FILE):
            print(f"{Colors.RED}{TARGET_GPS_FILE} not found{Colors.ENDC}")
            return False

        with open(TARGET_GPS_FILE) as f:
            entries = json.load(f)

        # filter + pick highest confidence
        matches = [e for e in entries if e["class"].lower() == obj_class]
        if not matches:
            print(f"{Colors.RED}No '{obj_class}' objects in {TARGET_GPS_FILE}{Colors.ENDC}")
            return False

        target = max(matches, key=lambda e: e.get("confidence", 0))
        lat, lon = target["lat"], target["lon"]
        alt = target.get("alt") or TAKEOFF_ALT

        print(f"{Colors.BLUE}Going to {obj_class} "
              f"({lat:.6f}, {lon:.6f}, {alt} m)…{Colors.ENDC}")
        return await self.goto_position(lat, lon, alt)

    # ---------------------- Battery ----------------------------------


    async def battery_level(self):
        """Print a quick battery snapshot (percent + voltage)."""
        batt = await self.drone.telemetry.battery().__anext__()
        pct  = batt.remaining_percent
        volt = batt.voltage_v
        print(f"{Colors.BOLD}Battery:{Colors.ENDC} {pct:.1f}%   {volt:.2f} V")
    ...
    async def monitor_telemetry(self):
        ...
        try:
            while True:
                ...
                self.log(f"Battery: {battery.remaining_percent:.1f}%, Voltage={battery.voltage_v:.2f}V")
    ...
    async def goto_object(self, object_class="person"):
        ...
        return success
'''
    async def hover(self, duration=5.0):
        """Hover in place for specified duration"""
        print(f"{Colors.BLUE}🚁 Hovering for {duration} seconds...{Colors.ENDC}")
        ...
        # Set velocity to zero for hovering
        try:
            # Set current command
            self.command_active = True
            self.current_command = "hover"
            
            # Stream hover commands
            end_time = asyncio.get_event_loop().time() + duration
            ...
            print(f"{Colors.RED}❌ Hover command failed: {e._result.result}{Colors.ENDC}")
            self.command_active = False
            return False

    async def fly_square(self, side_length=10.0):
        """Fly a square pattern"""
        print(f"{Colors.BLUE}🔲 Flying square pattern with {side_length}m sides...{Colors.ENDC}")
        
        # Fly forward, right, backward, left to make a square
        await self.move_direction("forward", side_length)
        await self.move_direction("right", side_length)
        await self.move_direction("backward", side_length)
        await self.move_direction("left", side_length)
        
        print(f"{Colors.GREEN}✅ Square pattern complete{Colors.ENDC}")
        return True

    async def record_path(self, duration=60):
        """Record current flight path for later playback"""
        print(f"{Colors.BLUE}📝 Recording flight path for {duration} seconds...{Colors.ENDC}")
        
        self.recorded_positions = []
        self.is_recording_path = True
        
        # Create recording task
        self.recording_task = asyncio.create_task(self._record_position_task(duration))
        
        return True
        
    async def _record_position_task(self, duration):
        """Background task to record positions"""
        try:
            end_time = asyncio.get_event_loop().time() + duration
            
            while asyncio.get_event_loop().time() < end_time and self.is_recording_path:
                position = await self.drone.telemetry.position().__anext__()
                attitude = await self.drone.telemetry.attitude_euler().__anext__()
                
                # Record position and attitude
            ...
            self.is_recording_path = False
            print(f"{Colors.GREEN}✅ Recorded {len(self.recorded_positions)} positions{Colors.ENDC}")
            
        except Exception as e:
            print(f"{Colors.RED}❌ Recording error: {e}{Colors.ENDC}")
            self.is_recording_path = False

    async def play_recorded_path(self):
        """Fly the recorded path"""
        if not self.recorded_positions:
            print(f"{Colors.RED}❌ No recorded path available{Colors.ENDC}")
            return False
            
        print(f"{Colors.BLUE}🔄 Playing back recorded path ({len(self.recorded_positions)} waypoints)...{Colors.ENDC}")
        
        # Convert recorded positions to mission items
        mission_items = []
        ...
        try:
            ...
            print(f"{Colors.GREEN}✅ Playback mission started{Colors.ENDC}")
            return True
            
        except Exception as e:
            print(f"{Colors.RED}❌ Playback failed: {e}{Colors.ENDC}")
            return False

    async def set_geofence_road(self, width=20.0, length=100.0):
        """Set geofence along the road"""
        print(f"{Colors.BLUE}🔒 Setting up geofence along road...{Colors.ENDC}")
        
        try:
            # Get current position as reference for road
            position = await self.drone.telemetry.position().__anext__()
            attitude = await self.drone.telemetry.attitude_euler().__anext__()
            ...
            # Calculate road points (assuming drone is aligned with road)
            # Road centerline is along drone's heading
            lat = position.latitude_deg
            lon = position.longitude_deg
            ...
            # Road length ahead and behind
            half_length = length / 2
            
            # Convert meters to degrees for latitude/longitude
            lat_per_meter = 1.0 / 111111.0
            lon_per_meter = 1.0 / (111111.0 * math.cos(math.radians(lat)))
            
            # Calculate corner points of the road
            # North = cos(yaw), East = sin(yaw)
            # Right = cos(yaw + 90°) = -sin(yaw), sin(yaw + 90°) = cos(yaw)
            
            # Front left
            front_left_lat = lat + half_length * math.cos(yaw_rad) * lat_per_meter - half_width * math.sin(yaw_rad) * lat_per_meter
            front_left_lon = lon + half_length * math.sin(yaw_rad) * lon_per_meter + half_width * math.cos(yaw_rad) * lon_per_meter
            
            # Front right
            front_right_lat = lat + half_length * math.cos(yaw_rad) * lat_per_meter + half_width * math.sin(yaw_rad) * lat_per_meter
            front_right_lon = lon + half_length * math.sin(yaw_rad) * lon_per_meter - half_width * math.cos(yaw_rad) * lon_per_meter
            
            # Back right
            back_right_lat = lat - half_length * math.cos(yaw_rad) * lat_per_meter + half_width * math.sin(yaw_rad) * lat_per_meter
            back_right_lon = lon - half_length * math.sin(yaw_rad) * lon_per_meter - half_width * math.cos(yaw_rad) * lon_per_meter
            
            # Back left
            back_left_lat = lat - half_length * math.cos(yaw_rad) * lat_per_meter - half_width * math.sin(yaw_rad) * lat_per_meter
            back_left_lon = lon - half_length * math.sin(yaw_rad) * lon_per_meter + half_width * math.cos(yaw_rad) * lon_per_meter
            
            # Create polygon
            points = [
                Point(front_left_lat, front_left_lon),
                Point(front_right_lat, front_right_lon),
                Point(back_right_lat, back_right_lon),
                Point(back_left_lat, back_left_lon)
            ]
            
            # Upload geofence
            await self.drone.geofence.upload_geofence([points])
            
            print(f"{Colors.GREEN}✅ Geofence set around road ({width}m x {length}m){Colors.ENDC}")
            return True
            
        except Exception as e:
            print(f"{Colors.RED}❌ Geofence setting failed: {e}{Colors.ENDC}")
            return False

    async def patrol_road(self, length=50.0, passes=2):
        """Patrol back and forth along the road"""
        print(f"{Colors.BLUE}🚓 Starting road patrol for {passes} passes...{Colors.ENDC}")
        ...
        try:
            ...
            # Calculate half length to go forward and backward
            half_length = length / 2
            
            # Convert meters to degrees for latitude/longitude
            lat_per_meter = 1.0 / 111111.0
            lon_per_meter = 1.0 / (111111.0 * math.cos(math.radians(position.latitude_deg)))
            
            # Calculate end points
            forward_lat = position.latitude_deg + half_length * math.cos(yaw_rad) * lat_per_meter
            forward_lon = position.longitude_deg + half_length * math.sin(yaw_rad) * lon_per_meter
            
            backward_lat = position.latitude_deg - half_length * math.cos(yaw_rad) * lat_per_meter
            backward_lon = position.longitude_deg - half_length * math.sin(yaw_rad) * lon_per_meter
            ...
            for i in range(passes):
                ...
                mission_items.append(MissionItem(
                    ...
                    float('nan'),  # Camera photo interval
                    1.0,  # Acceptance radius
                    attitude.yaw_deg + 180.0,  # Yaw (face opposite direction)
                    float('nan')  # Camera photo distance
                ))
            ...
            print(f"{Colors.GREEN}✅ Road patrol mission started ({len(mission_items)} waypoints){Colors.ENDC}")
            return True
            
        except Exception as e:
            print(f"{Colors.RED}❌ Road patrol failed: {e}{Colors.ENDC}")
            return False

    async def inspect_all_vehicles(self):
        """Inspect all detected vehicles in sequence"""
        print(f"{Colors.BLUE}🔍 Starting vehicle inspection sequence...{Colors.ENDC}")
        
        try:
            # Read detection file
            if not os.path.exists(DETECTION_FILE):
                print(f"{Colors.RED}❌ Detection file not found: {DETECTION_FILE}{Colors.ENDC}")
                return False
                
            with open(DETECTION_FILE, "r") as f:
                detections = json.load(f)
                
            # Filter for vehicles (car, truck)
            vehicles = []
            for detection in detections:
                if detection["class"] in ["car", "truck"] and detection["confidence"] > 0.5:
                    vehicles.append(detection)
                    
            if not vehicles:
                print(f"{Colors.RED}❌ No vehicles detected with sufficient confidence{Colors.ENDC}")
                return False
                
            print(f"{Colors.GREEN}✅ Found {len(vehicles)} vehicles to inspect{Colors.ENDC}")
            ...
            for i, vehicle in enumerate(vehicles):
                print(f"\n{Colors.BLUE}🚗 Inspecting vehicle {i+1}/{len(vehicles)} ({vehicle['class']}){Colors.ENDC}")
                
                # Very simplified conversion (not accurate, just for example)
                lat_offset = (vehicle['x'] - 320) / 10000.0
                lon_offset = (vehicle['y'] - 240) / 10000.0
                
                vehicle_lat = position.latitude_deg + lat_offset
                vehicle_lon = position.longitude_deg + lon_offset
                
                # Go to vehicle position
                print(f"{Colors.YELLOW}⏺️ Moving to vehicle at Lat={vehicle_lat:.6f}, Lon={vehicle_lon:.6f}{Colors.ENDC}")
                await self.goto_position(vehicle_lat, vehicle_lon)
                
                # Wait to reach position
                await asyncio.sleep(5)
                
                # Hover and inspect
                await self.hover(5.0)
                
                # Orbit around vehicle
                print(f"{Colors.YELLOW}⏺️ Orbiting around vehicle{Colors.ENDC}")
                await self.orbit(radius=5.0, center_lat=vehicle_lat, center_lon=vehicle_lon)
                
                # Allow some time for orbit to start
                await asyncio.sleep(10)
                
                # Stop orbit and move to next vehicle
                await self.stop_current_command()
                
            print(f"{Colors.GREEN}✅ Vehicle inspection complete{Colors.ENDC}")
            return True
            
        except Exception as e:
            print(f"{Colors.RED}❌ Vehicle inspection failed: {e}{Colors.ENDC}")
            return False
    ...
    async def print_status(self):
        ...
        try:
            ...
            print(f"  Level: {battery.remaining_percent:.1f}%")
...
{Colors.BOLD}Basic Commands:{Colors.ENDC}
  ...
  {Colors.GREEN}battery{Colors.ENDC}                  - Show current battery % and voltage
...
{Colors.BOLD}Movement Commands:{Colors.ENDC}
  ...
  {Colors.GREEN}turn_cw{Colors.ENDC} <degrees>        - Turn clockwise specified degrees
  {Colors.GREEN}turn_ccw{Colors.ENDC} <degrees>       - Turn counter-clockwise specified degrees

{Colors.BOLD}Navigation Commands:{Colors.ENDC}
  ...
  {Colors.GREEN}hover{Colors.ENDC} [duration]         - Hover in place for specified duration

{Colors.BOLD}Object Detection Commands:{Colors.ENDC}
  ...
  {Colors.GREEN}inspect_vehicles{Colors.ENDC}         - Inspect all detected vehicles
  {Colors.GREEN}goto_object{Colors.ENDC} <class> [n]  - Fly to object <class> (n‑th by confidence) using target_gps.json

{Colors.BOLD}Pattern Commands:{Colors.ENDC}
  {Colors.GREEN}square{Colors.ENDC} [size]            - Fly a square pattern
  {Colors.GREEN}patrol{Colors.ENDC} [length] [passes] - Patrol along the road

{Colors.BOLD}Path Recording Commands:{Colors.ENDC}
  {Colors.GREEN}record{Colors.ENDC} [duration]        - Record flight path for playback
  {Colors.GREEN}play{Colors.ENDC}                     - Fly recorded path

{Colors.BOLD}Geofence Commands:{Colors.ENDC}
{Colors.GREEN}fence_road{Colors.ENDC} [width] [length] - Set geofence along the road
...
async def main():
    ...
    print(f"{Colors.BOLD}{Colors.HEADER}==== MAVSDK-Python Advanced Drone Controller ===={Colors.ENDC}")
    ...
    while True:
        try:
            ...
            if cmd == "help":
            ...
            elif cmd in ["forward", "backward", "left", "right", "up", "down", "yaw_left", "yaw_right", "turn_cw", "turn_ccw"]:
                if len(parts) < 2:
                ...
                await controller.battery_level()


            elif cmd == "goto_object":
                if len(parts) < 2:
                    print("Usage: goto_object <class>")
                    continue
                await controller.goto_object(parts[1])
                ...
                await controller.hover(duration)
                

                
            elif cmd == "inspect_vehicles":
                await controller.inspect_all_vehicles()
                
            elif cmd == "square":
                size = float(parts[1]) if len(parts) > 1 else 10.0
                await controller.fly_square(size)
                
            elif cmd == "patrol":
                length = float(parts[1]) if len(parts) > 1 else 50.0
                passes = int(parts[2]) if len(parts) > 2 else 2
                await controller.patrol_road(length, passes)
                
            elif cmd == "record":
                duration = float(parts[1]) if len(parts) > 1 else 60.0
                await controller.record_path(duration)
                
            elif cmd == "play":
                await controller.play_recorded_path()
                
            elif cmd == "fence_road":
                width = float(parts[1]) if len(parts) > 1 else 20.0
                length = float(parts[2]) if len(parts) > 2 else 100.0
                await controller.set_geofence_road(width, length)
                ...
                if controller.recording_task and not controller.recording_task.done():
                    controller.recording_task.cancel()
